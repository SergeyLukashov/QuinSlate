...
## What
Ensure only one running copy of Jott exists at any time. A second launch
must hand off to the existing instance and exit immediately.

## Behaviour

- If Jott is not running: start normally.
- If Jott is already running: bring the existing panel to focus, then exit
...
On startup, before any UI is initialised, attempt to acquire a named mutex:
    ...
    Mutex mutex = new Mutex(true, "Local\\JottSingleInstance", out bool acquired);
...
Signal the existing instance by posting a custom message to its window:
    ...
    const int WM_JOTT_ACTIVATE = 0x8001;  // WM_APP + 1

    HWND existing = FindWindow(null, "JottMainWindow");
    if (existing != IntPtr.Zero)
        PostMessage(existing, WM_JOTT_ACTIVATE, 0, 0);

    Environment.Exit(0);

The existing instance handles `WM_JOTT_ACTIVATE` in its Win32 message
...
- `FindWindow` matches on window title. The main window must have a stable,
  non-localised title string (`"JottMainWindow"`) that is never shown in
  the UI.
